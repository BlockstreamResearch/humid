 ...
 * the party the rule is written against. So a rule that fails refuses, and a rule this
 * runtime cannot read refuses too: reading half a rule permits exactly what it was written
 * to prevent.
 *
 * Only `arithmetic` rules, which is every rule the corpus contains. A `simplicity_hl` rule
 * is a contract to execute rather than a condition to evaluate, and `utxo_exists` is a chain
...
export function checkValidations(
	...
	for (const declared of asArray(action.node.validations)) {
		...
		if (rule?.type !== "arithmetic") {
			return {
				reason:
					`The rule ${id} is a ${String(rule?.type)} check, which this wallet cannot make. ` +
					"It will not build a transaction its own protocol asked it to check first.",
			};
		}
 ...
 *
 * The protocol's own message when it wrote one, because it knows what the rule means and
 * this wallet does not — attributed to the protocol rather than stated as the wallet's own
 * finding, which is the same rule that governs every other piece of site-authored text.
 */
