 ...
 *
 * A deployment mostly wires a compile parameter to a name, and the name carries the format's
 * own declared type. Some parameters are wired to a bare value instead — a number, or one of
 * the two words a flag is written as. That position declares nothing, because it is a
 * deployment's wiring rather than a list of parameters, so the value arrives with no type and
 * there is nothing to encode it against.
 *
 ...
 * `simplicityhl` 0.6.0 inserts the parameter into the program's global map under the expected
 * type of the expression it stands in for (`src/ast.rs` L1346-1350). So a parameter's type is
 * not written down anywhere in the source; it is a result of analysing the source, and the
 * only thing that can state it is the compiler.
 *
 * That is why this takes the types as given rather than reading the contract text. A reader
 * that recovered them from the source would be reimplementing the type checker, and the
 * failure mode of getting one wrong is a value encoded at a width nobody stated.
 */
 ...
 *
 * The source path is carried because a refusal has to say which contract decided the answer.
 * Two utxo types can wire the same parameter name into different contracts, and "the contract
 * does not take a value there" is only actionable if the reader knows which one is meant.
 */
 ...
 *
 * The entry names on the right are the format's declared-type names, and for these five they
 * are spelled identically to the compiler's. That is not relied upon quietly: `encodeContractLiteral`
 ...
 *
 * The list is closed for the same reason the encoding list is closed. A type nobody has mapped
 * is refused by name rather than passed through.
 */
 ...
 *
 * Nothing here looks at the value. `"1"` is not read as a small number, `"false"` is not read
 * as a flag, and a run of sixty-four hexadecimal characters is not read as a hash. The type
...
export function encodeContractLiteral(
	...
	// typed differently from what the contract declared would be refused there rather than
	// built wrongly. Refusing it here says which parameter and which contract, which the
	// compiler's own message does not.
