 ...
 * than passed through, because the compiler will accept almost anything shaped like a value
 * and hand back a perfectly valid address for the wrong contract. Nothing here is derived
 * from a type's name or from what a manifest says it means; each entry below records what was
 * measured against the compiler and, where the value's form is not self-evident, against a
 * contract executing on a transaction that carries the value.
 *
 * Two facts from simplicityhl 0.6.0 decide the whole table.
 *
 * A value is parsed as an expression and const-analysed against the declared type
 * (`src/serde.rs` `ValueMapVisitor`, `src/value.rs` L789). So `0x…` is a hexadecimal literal
 * and a run of digits is a decimal one, and they are different literals rather than two
 * spellings of one.
 *
 * A hexadecimal literal must be exactly the type's width — `src/value.rs` L629-640 rejects a
 * string whose length is not `byte_width * 2`. That is what makes hex safe for the fixed-width
 * types and unsafe for the integers: `0x1000000000000000` is a legal `u64` and is not the
 ...
 *
 * Every type the published corpus declares at a compile-parameter site is here. The ones it
 * does not declare are absent on purpose — see `unencodableReason` for what each of them
 * would need before it could be added.
 */
const PARAM_TYPES: Record<string, ParamType> = {
	 ...
	 *
	 * This is the one entry in the table where passing the value through would compile, derive
	 * an address, and be wrong. A covenant reads an asset with `jet::input_amount` or
	 * `jet::output_amount` and compares the bits it gets against its baked-in parameter — every
	 * asset-id parameter in the corpus is used that way, in `asset_auth.simf`,
	 * `asset_auth_vault.simf` and `lending.simf` — and what those jets report is the asset as
	 * the transaction commits it, which is the reverse of the form everything states an id in.
	 *
	 * Measured rather than reasoned: `asset_auth.simf` was compiled both ways and dry-run
	 * against a transaction whose input and output carried the asset. The committed form
	 * executed; the stated form failed inside the jet. Both compiled, and both produced an
	 * address.
	 *
	 * Everything on this side of the wallet states an id the way a person reads it — the
	 * chain reader turns each one round on the way in (`chain/txOut.ts`), and an id in a
	 * document is compared against that form — so the turn belongs here, at the one place a
	 * stated id becomes committed bytes.
	 */
	"liquid.asset_id": {
		bytes: 32,
		compiler: "u256",
	 ...
	 *
	 * A number is hex-prefixed nowhere here, and that is the point rather than a convention.
	 * The compiler reads `0x…` as a hexadecimal literal of exactly the type's width, so a
	 * decimal amount that happens to be sixteen characters long is a legal `u64` hexadecimal
	 * literal standing for an entirely different number. It compiles, derives an address, and
	 * says nothing. Measured: `1000000000000000` and `0x1000000000000000` compiled
	 * `issuance_factory.simf` to two different scriptPubKeys.
	 *
 ...
 * heard of, because they call for different things: the first waits on a decision about what
 * it means, and the second is usually a typo in the document.
 */
