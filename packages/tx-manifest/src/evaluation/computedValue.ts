 ...
 * below; every expression that is more than a single term contains at least one. That is exact
 * for this grammar rather than a heuristic, and it is the reason a literal survives.
 */
 ...
 * legal — a formula at a position that cannot see an input's attributes is refused for naming
 * one rather than for arithmetic — and passing the position through is what keeps that true of
 * a computed value as much as of a plain reference.
 */
