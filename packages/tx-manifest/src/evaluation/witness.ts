 ...
 *
 * The type and the value stay text. A SimplicityHL literal is the compiler's to parse, and
 * this wallet does not implement that language — reading `Right(Left(()))` as a structure here
 * would be a second opinion about which branch a contract runs, given by the one component
 ...
 *
 * Resolved after the hooks rather than beside the covenant, because one published protocol
 * selects a branch by a field of its own deployment: the value is a literal with a name inside
 * it, and the name is not known until everything that writes fields has run.
 */
 ...
 *
 * Thirty of the corpus's thirty-one stated values are literals with nothing to replace. The
 * thirty-first selects a branch carrying a field of its own deployment, so a runtime that
 * skipped this would hand the compiler the word `instance.CURRENT_DEBT` and be told the
 * literal does not parse — which is true and says nothing about why.
 *
...
function fill(
	...
	const filled = value.replaceAll(NAMED, (text) => {
		...
		// reference here: the site accepts three namespaces and nothing else, so leaving it
		// alone would hand the compiler a word it cannot parse and claiming it resolves would
		// be worse. Refusing names it.
