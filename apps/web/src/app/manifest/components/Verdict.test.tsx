...
// AC-01, AC-02, AC-03 and AC-06's first half, at the only place they can be checked: the text
// a reader actually meets. Rendered to a string rather than to a DOM, because this repository
// has no DOM in its tests and react-dom is already here — the assertions below are about words
// on a screen, and a string carries those.
...
describe("what was never asked, beside the answer", () => {
	...
	// AC-02. Nothing that says a check was not made may hide behind a click: the absence of a
	...
	test("tells a reader who can still answer that they can", () => {
		const html = render({ ...NOTHING_ASKED, refusal: undefined, skipped: ["foreign-asset"] });

		expect(html).toContain("Choose one above and they run");
	});
});

describe("the runtime's own names for its refusals", () => {
	// AC-03. A person cannot act on a reject token; they can act on the sentence beside it. The
...
describe("older spellings, said once", () => {
	// AC-06's first half. A renaming that succeeded changed nothing about the answer, so what is
