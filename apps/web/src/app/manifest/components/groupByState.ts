 ...
 * field here. `acted-on` trails, because it is the state of a field that works — for the
 * deployed lending protocol that is most of the document, and putting them before the rest
 * buries the few worth reading.
 */
 ...
 * count is visible without clicking. What is removed is the default of meeting hundreds of rows
 * that each say "this field works" before reaching the few that say anything else.
 */
 ...
 *
 * A row per position is one row per key per place that key appears, which for the published
 * protocols is hundreds of rows over a few dozen distinct keys. Nothing there is wrong and
 * nothing is readable, because the repetition is inherent to the shape of the data rather than
 * to anything the document did.
 *
 ...
 *
 * A function rather than a few lines inside the component because it is the only decision that
 * surface makes: everything else there is layout, and a decision left inside JSX is a decision
 * nothing can check.
 */
