 ...
 * all seven that the format defines and this wallet does not implement are in that position.
 * Every published protocol therefore inspects clean while the seven stand, which is why this
 * is a page rather than a section beside a box someone pastes into.
 *
 * It reads nothing from that page and nothing from anywhere else. Its whole content is the
 * runtime's own construct table, so it cannot describe a wallet that differs from the one
 * that runs.
 */
 ...
 *
 * Counted from the table rather than written down, so the sentence cannot fall behind the
 * thing it describes — which is the same reason this page exists at all.
 */
